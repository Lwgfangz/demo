
// To check if a library is compiled with CocoaPods you
// can use the `COCOAPODS` macro definition which is
// defined in the xcconfigs so it is available in
// headers also when they are imported in the client
// project.


// NSLogger
#define COCOAPODS_POD_AVAILABLE_NSLogger
#define COCOAPODS_VERSION_MAJOR_NSLogger 1
#define COCOAPODS_VERSION_MINOR_NSLogger 2
#define COCOAPODS_VERSION_PATCH_NSLogger 0

// xProject
#define COCOAPODS_POD_AVAILABLE_xProject
#define COCOAPODS_VERSION_MAJOR_xProject 1
#define COCOAPODS_VERSION_MINOR_xProject 0
#define COCOAPODS_VERSION_PATCH_xProject 1

